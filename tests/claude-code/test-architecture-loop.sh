#!/usr/bin/env bash
# Test: architecture-loop skill behavioral verification
# Verifies that Claude has internalized the architecture loop's rules
set -euo pipefail
...
echo "=== Test: architecture-loop skill ==="
echo ""

# Test 1: Skill recognition and purpose
echo "Test 1: Skill recognition..."

output=$(run_claude "What is the arxitect:architecture-loop skill? Describe its purpose and key phases briefly." 30)
...
output=$(run_claude "In the arxitect:architecture-loop skill, which reviewers are used during the review phase? Name all of them." 30)
...
output=$(run_claude "In arxitect:architecture-loop, how many iterations does the feedback loop run before stopping? What happens when it reaches that limit?" 30)
...
output=$(run_claude "In arxitect:architecture-loop, is it ever okay to skip the architecture review step after implementation? Even if the code looks good?" 30)
...
output=$(run_claude "In the arxitect:architecture-loop skill, what are the implementation strategies and in what order are they checked?" 30)
...
output=$(run_claude "In arxitect:architecture-loop, how are review findings tracked across iterations? What happens if a fixed finding reappears?" 30)
...
output=$(run_claude "In arxitect:architecture-loop, if a finding was fixed in iteration 1 but reappears in iteration 2, what happens to its severity?" 30)
...
output=$(run_claude "In arxitect:architecture-loop, does the implementer agent receive design guidelines? Where are they defined?" 30)
...
output=$(run_claude "In the architecture-loop, can the architecture reviewers modify files during review?" 30)

assert_contains "$output" "no\|read.only\|not\|never\|cannot" "Reviewers are read-only" || exit 1

echo ""

echo "=== All architecture-loop skill tests passed ==="
